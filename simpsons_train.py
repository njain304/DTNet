...
class FaceTest(BaseTrain):
    ...
    def init_data_loaders(self):
        celeb_transform = transforms.Compose(
            [data.ResizeTransform(96),
             data.TanhNormalize()])  # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        simpsons_transform = transforms.Compose(
            [data.ResizeTransform(96),
             data.TanhNormalize()])  # transforms.Normalize((0.2411, 0.1801, 0.1247), (0.3312, 0.2672, 0.2127))])
        ...
        t_train_set = simpsons.Simpsons(data_dir='./data/simpsons', split='train', transform=simpsons_transform)
    ...
    def visualize_single_batch(self):
        ...
        unnorm_ms = data.TanhDeNormalize()  # ((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
        unnorm_emoji = data.TanhDeNormalize()  # ((0.2411, 0.1801, 0.1247), (0.3312, 0.2672, 0.2127))
    ...
    def init_optimizer(self):
        ...
        generator_decay = 0
        self.g_optimizer = optim.Adam(self.model['G'].parameters(), lr=generator_learning_rate, betas=(0.9, 0.999), weight_decay=generator_decay)

        discriminator_learning_rate = 5e-4
        discriminator_decay = 0
        self.d_optimizer = optim.Adam(self.model['D'].parameters(), lr=discriminator_learning_rate, betas=(0.9, 0.999), weight_decay=discriminator_decay)
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(l):
                ...
                if i % visualize_batches == 0:
                    ...
                    print("d_src_loss: %f, g_src_loss %f, lconst_src_loss %f d_trg_loss %f, g_trg_loss %f" % (
                    d_src_loss, g_src_loss, lconst_src_loss, d_trg_loss, g_trg_loss))
