...
class G(nn.Module):
    ...
    def forward(self, input):
        conv_in = input.view(input.size()[0], 864, 1, 1)
...
class D(nn.Module):
    def __init__(self, channels, alpha=0.2):
        ...
        self.block = nn.Sequential(
            ConvBNLRelu(3, self.channels, self.alpha),
            ConvBNLRelu(self.channels, self.channels * 2, self.alpha),
            ConvBNLRelu(self.channels * 2, self.channels * 4, self.alpha),
